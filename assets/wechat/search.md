...
|------|-----|
| control_type | Edit |
| class | mmui::SearchCtrl |
| automation_id | search_input_field |
...
# y的位置要往下一些，确保点击到搜索框内部
python scripts\winguictl.py action --window-id <wx_window_id> click --relative-x <search_x> --relative-y <search_y+10>
```
...
# 2. 定位搜索框
python scripts\winguictl.py find --window-id <wx_window_id> uia --automation-id search_input_field
